/*
=============================================================================== 
Program Name    : Binary Search Tree - Preorder Traversal
Program Number  : 05 
File Name       : Tree_05.cpp
Description     : This program implements Preorder Traversal of a Binary
                  Search Tree using C++. Preorder Traversal visits the
                  nodes in Data, Left, Right (DLR) order.

Date            : August 2026
Language        : C++
Author          : Ishwari Vijaykumar Surve
===============================================================================

Concepts Used:
1. Binary Search Tree (BST)
2. Preorder Traversal
3. Recursion
4. Class and Object
5. Self-Referential Structure
6. Pointers

Key Learning Points:
- Understand Preorder Traversal of a Binary Search Tree.
- Learn the DLR traversal technique.
- Understand recursive traversal of Tree nodes.
- Learn how the root node is processed before its subtrees.
- Understand the use of a wrapper function for traversal.

===============================================================================
*/

#include<iostream>
using namespace std;

#pragma pack(1)

struct node
{
    int data;
    struct node *lchild;
    struct node *rchild;
};

class BST
{
private:
    struct node *first;

public:
    BST();
    void Insert(int iNo);

    void Preorder(struct node *first);
    void Display();
};

// Constructor
BST::BST()
{
    first = NULL;
}

// Insert Node
void BST::Insert(int iNo)
{
    struct node *newn = NULL;
    struct node *temp = NULL;

    newn = new node;

    newn->data = iNo;
    newn->lchild = NULL;
    newn->rchild = NULL;

    if(first == NULL)
    {
        first = newn;
    }
    else
    {
        temp = first;

        while(1)
        {
            if(iNo > temp->data)
            {
                if(temp->rchild == NULL)
                {
                    temp->rchild = newn;
                    break;
                }

                temp = temp->rchild;
            }
            else if(iNo < temp->data)
            {
                if(temp->lchild == NULL)
                {
                    temp->lchild = newn;
                    break;
                }

                temp = temp->lchild;
            }
            else
            {
                cout<<"Duplicate element not allowed\n";
                delete newn;
                break;
            }
        }
    }
}

// Preorder Traversal
// D L R
void BST::Preorder(struct node *first)
{
    if(first != NULL)
    {
        cout<<first->data<<"\n";

        Preorder(first->lchild);

        Preorder(first->rchild);
    }
}

// Wrapper Function
void BST::Display()
{
    Preorder(first);
}

int main()
{
    BST bobj;

    bobj.Insert(11);
    bobj.Insert(5);
    bobj.Insert(17);
    bobj.Insert(4);
    bobj.Insert(7);
    bobj.Insert(15);
    bobj.Insert(21);

    cout<<"Preorder Traversal :\n";

    bobj.Display();

    return 0;
}

/*
===============================================================================
Expected Output:
===============================================================================

Preorder Traversal :
11
5
4
7
17
15
21

===============================================================================
Memory Representation:
===============================================================================

                         first
                           |
                           ▼
                       +-------+
                       |  11   |
                       +---+---+
                          / \
                         /   \
                        ▼     ▼
                    +-----+ +-----+
                    |  5  | | 17  |
                    +--+--+ +--+--+
                      / \     / \
                     /   \   /   \
                    ▼     ▼ ▼     ▼
                  +---+ +---+ +---+ +---+
                  | 4 | | 7 | |15 | |21 |
                  +---+ +---+ +---+ +---+

Preorder Traversal: Data → Left → Right

Traversal Sequence:

11 → 5 → 4 → 7 → 17 → 15 → 21

===============================================================================
Algorithm:
===============================================================================

1. Start the program.
2. Create a Binary Search Tree.
3. Insert the required elements into the BST.
4. Call the Display() function.
5. Display() calls the Preorder() function with the root node.
6. In Preorder(), first display the data of the current node.
7. Recursively visit the left subtree.
8. Recursively visit the right subtree.
9. Continue the process until all nodes are visited.
10. Stop the program.

===============================================================================
Program Flow:
===============================================================================

                         Start
                           │
                           ▼
                    Create BST Object
                           │
                           ▼
                    Insert BST Nodes
                           │
                           ▼
                     Call Display()
                           │
                           ▼
                    Call Preorder()
                           │
                           ▼
                    Display Node Data
                           │
                           ▼
                  Traverse Left Subtree
                           │
                           ▼
                 Traverse Right Subtree
                           │
                           ▼
                    All Nodes Visited?
                       /          \
                     No            Yes
                     │              │
                     └─── Repeat    ▼
                                  Stop

===============================================================================
Time Complexity:
===============================================================================

Insert() - Best Case    : O(1)
Insert() - Average Case : O(log n)
Insert() - Worst Case   : O(n)

Preorder() - Best Case   : O(n)
Preorder() - Average Case: O(n)
Preorder() - Worst Case  : O(n)

Where n is the number of nodes in the BST.

===============================================================================
Space Complexity:
===============================================================================

Preorder() Auxiliary Space : O(h)

Where h is the height of the Tree.

For a balanced Tree : O(log n)
For a skewed Tree   : O(n)

Total Space : O(n)

===============================================================================
Conclusion:
===============================================================================

Program 005 implements Preorder Traversal of a Binary Search Tree using
C++. The traversal follows the Data, Left, Right (DLR) order. The current
node is processed first, followed by the left subtree and then the right
subtree. Recursion is used to traverse the complete Tree.

===============================================================================
*/
